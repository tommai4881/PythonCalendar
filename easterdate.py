...
def easter(y, cal = True):
    ...
    p = (13 + 8 * k) // 25
    q = k // 4
    if cal: # Việt Nam không dùng lịch Julian nên ta quy ước lấy 1582.
    ...
    if d == 29 or (d == 28 and a > 10): 
        # Xử lý edge case đ/v epact 24 và 25 đen (25 chỉ đen khi a > 10)
        d -= 1
    e = (2 * b + 4 * c + 6 * d + n) % 7
    March = d + e + 22
    April = d + e - 9
...
if __name__ == "__main__": # Ví dụ
    print(easter(date.today().year)) # Lễ Phục Sinh năm nay